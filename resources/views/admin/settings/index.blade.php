            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div>
                            <form id="maintenance-toggle-form" action="{{ route('admin.settings.update') }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="maintenance_enabled" name="maintenance_enabled" value="1" {{ optional($settings)->maintenance_enabled ? 'checked' : '' }} onchange="document.getElementById('maintenance-toggle-form').submit()">
                                    <label class="form-check-label" for="maintenance_enabled">{{ optional($settings)->maintenance_enabled ? 'Bật' : 'Tắt' }}</label>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" name="maintenance_ends_at" class="form-control" value="{{ optional($settings->maintenance_ends_at)->format('Y-m-d\TH:i') ?? '' }}">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Thông báo bảo trì</label>
                            <textarea class="form-control" name="maintenance_message" rows="3" placeholder="Website đang trong quá trình bảo trì. Vui lòng quay lại sau.">{{ optional($settings)->maintenance_message }}</textarea>
                        </div>
